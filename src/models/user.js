...
//
const insertUser = async (body, hashedPassword) => {
  try {
    ...
    return await db.execute(
      ...
      [username, fullname, address, sex, hashedPassword, email, +groupid]
    );
  } catch (error) {
    throw error;
  }
};
const listUser = async (body, hashedPassword) => {
  try {
    const db = await connectDb();
    return await db.execute(
...
const delUser = async (username) => {
  try {
    const db = await connectDb();
    return await db.execute("DELETE  FROM `users`  WHERE users.username = ?", [
      username,
    ]);
  } catch (error) {
    console.log("err ", error);
...
export { insertUser, listUser, detailUser, updateUser, delUser, login };
